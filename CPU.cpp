...
void CPU::execute(u32 cycles, bool useCycles, Memory& mem) {
	while ((cycles > 0 || !useCycles) && PC != 0xFFFF) {
		...
		case(INS_BCC): { branch(C == 0, cycles, mem); }break;
		case(INS_BCS): { branch(C == 1, cycles, mem); }break;
		case(INS_BEQ): { branch(Z == 1, cycles, mem); }break;
		case(INS_BMI): { branch(N == 1, cycles, mem); }break;
		case(INS_BNE): { branch(Z == 0, cycles, mem); }break;
		case(INS_BPL): { branch(N == 0, cycles, mem); }break;
		case(INS_BVC): { branch(V == 0, cycles, mem); }break;
		case(INS_BVS): { branch(V == 1, cycles, mem); }break;
